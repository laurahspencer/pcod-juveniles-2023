...
test <- "GeneID:132464016" # ZP3 gene on P. cod genome 
...
DEGs.4David.uniprot <- list(
  ...
  "Cold (0degC)" = 
    res.0 %>% as.data.frame() %>% rownames_to_column("gene_pcod") %>%
    filter(abs(select(., contains("log2FoldChange")))>0.5) %>%
    left_join(pcod.blast) %>%
    dplyr::select(spid) %>% na.omit() %>% unlist() %>% as.vector(),
  
  "Cool (5degC)" = 
    res.5 %>% as.data.frame() %>% rownames_to_column("gene_pcod") %>%
    filter(abs(select(., contains("log2FoldChange")))>0.5) %>%
    left_join(pcod.blast) %>%
    dplyr::select(spid) %>% na.omit() %>% unlist() %>% as.vector(),
  
  "Warm (16degC)" = 
    res.16 %>% as.data.frame() %>% rownames_to_column("gene_pcod") %>%
    filter(abs(select(., contains("log2FoldChange")))>0.5) %>%
    left_join(pcod.blast) %>%
    dplyr::select(spid) %>% na.omit() %>% unlist() %>% as.vector(),
...
# Write tab-delimited file with each column containing Uniprot IDs for each set of DEGs 
write_delim(x = ldply(DEGs.4David.uniprot, rbind) %>% t() %>% as.data.frame(), delim = "\t", col_names = F, na="", 
