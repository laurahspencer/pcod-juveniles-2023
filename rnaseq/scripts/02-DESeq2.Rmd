...
fragments <- data.frame(rowSums(counts.ts)) %>% 
  ...
  left_join(sample.info %>% dplyr::select(sample_name, sample_number, tank_number, temperature, tissue_type), by=c("sample"="sample_name")) %>%
...
counts.tk <- merge(x=sample.info[,c("sample_name", "temperature", "tissue_type", "tank_number", "temperature_tank")], by.x="sample_name", y=counts.ts, by.y="row.names") %>% 
...
<!-- # Controlling for fish length  -->

<!-- ## Examine length distributions by temperature -->
...
<!-- length.rnaseq %>% -->
<!--   clean_names() %>% -->
<!--   mutate_at(c("temperature", "p_h", "tank_number"), factor) %>%  -->
...
<!-- 2. Use edgeR to test for effects of length as a continuous explanatory variable using all 3 temperatures. -->
<!-- 3. Use edgeR to test for effects of length as a continuous explanatory variable without the 10C temperature. -->

<!-- ### 1. Genes associated with length using DESeq2 and all three temperatures  -->
...
<!-- ### 2. Genes associated with length using edgeR and all three temperatures  -->

<!-- ```{r} -->
<!-- # Re-organize counts dataframe to use in edgeR -->
<!-- counts.treat <- counts.ts %>% rownames_to_column("sample") %>% -->
<!--   left_join(sample.info[c("tank_number", "temperature", "ph", "sample_name", "length")], by=c("sample"="sample_name")) %>% -->
<!--   mutate(temperature=as.factor(paste(ph, temperature, sep="."))) %>% -->
...
<!--   select(sample, tank_number, temperature, ph, temperature, length, everything()) -->
...
<!--   dplyr::select(-tank_number, -temperature, -ph, -temperature, -length) %>% -->
<!--   t() %>% as.data.frame() -->
...
<!-- # Extract vectors of larval size (length) and temperature -->
<!-- temperature.length <- counts.treat$temperature -->
<!-- length <- counts.treat$length -->
<!-- temperature.length <- counts.treat$temperature -->

<!-- # Create a DGEList object  -->
<!-- y.length <- DGEList(counts.length, group=temperature.length) -->
...
<!--   left_join(counts.treat[c("sample", "temperature", "ph", "temperature", "length")],  -->
<!--             by=c("name"="sample")) -->
...
<!-- #design <- model.matrix(~ 0 + temperature)  -->
<!-- #colnames(design.length) <- levels(temperature) -->
...
<!-- # Create a DGEList object  -->
<!-- y.length.no10 <- DGEList(counts.length.no10, group=temperature.length.no10) -->
...
<!-- Consensus set of genes that are associated with length (across all temperatures) using DESeq2 and both edgeR analyses (all three temperatures, no 10C)  -->
...
<!--           rownames(DEGs.lin.length$table),  #From edgeR all 3 temperatures -->
...
<!-- ```{r} -->
<!-- dds.temperature.all <- DESeqDataSetFromMatrix(countData = counts.tk[-1:-4] %>% t(), -->
<!--                               colData = counts.tk[,"temperature", drop=FALSE] , -->
<!--                               design = ~ temperature) -->
<!-- dds.DESeq.temperature.all <- DESeq(dds.temperature.all) -->

<!-- vsd.temperature.all <- varianceStabilizingTransformation(dds.temperature.all, blind=FALSE) -->
<!-- ``` -->
...
```{r}
dds.temperature <- DESeqDataSetFromMatrix(countData = counts.tk[, !(colnames(counts.tk) %in% genes.length)][-1:-4] %>% t(),
                              colData = counts.tk[,"temperature", drop=FALSE] ,
                              design = ~ temperature)

dds.tank <- DESeqDataSetFromMatrix(countData = counts.tk[, !(colnames(counts.tk) %in% genes.length)][-1:-4] %>% t(),
                              colData = counts.tk[,"tank_number", drop=FALSE] ,
                              design = ~ tank_number)

dds.temperature.tank <- DESeqDataSetFromMatrix(countData = counts.tk[, !(colnames(counts.tk) %in% genes.length)][-1:-4] %>% t(),
...
dendo.df <- sample.info[match(cluster.most.variable$tree_row$labels, sample.info$sample_name),c("sample_name", "temperature", "tank_number")]
...
colorCodes.tank <- colorRampPalette(brewer.pal(12, "Paired"))(length(levels(sample.info$tank_number)))
...
counts.trans <- assay(vsd.temperature) %>% t() %>% as.matrix() %>% as.data.frame() %>% 
  rownames_to_column(var = "sample") %>% 
  left_join(sample.info[,c("sample_name", "temperature", "tank_number")], by = c("sample"="sample_name")) %>%
...
tab.expr.annot <- left_join(tab.expr, sample.info, by=c("sample.id"="sample_name")) %>% droplevels()
```
...
perm <- adonis(vsd.t ~ temperature, data=sample.info %>% remove_rownames() %>% column_to_rownames("sample_name"), permutations = 1000, method="bray")

# Pairwise comparisons
vsd.t <- sample.info %>% dplyr::select(sample_name, temperature, tank_number) %>% left_join(t(assay(vsd.temperature)) %>% as.data.frame() %>% rownames_to_column("sample_name"))
...
# sample.info %>% select(Sample, temperature) %>% left_join(counts.t %>% rownames_to_column("Sample")) %>% 
...
# sample.info %>% select(Sample, temperature) %>% left_join(counts.tk %>% rownames_to_column("Sample")) %>% 
...
a <-  plotCounts(dds.DESeq.temperature, gene=test, intgroup=c("temperature"), replaced = TRUE, returnData = TRUE) %>%
  rownames_to_column("sample") %>%
  left_join(sample.info[,c("sample_name", "tank_number", "temperature_tank")], by=c("sample"="sample_name")) %>%
...
samples.0 <- (sample.info[c("sample_name", "temperature")] %>% filter(temperature=="0"))$sample_name
samples.5 <- (sample.info[c("sample_name", "temperature")] %>% filter(temperature=="5"))$sample_name
samples.9 <- (sample.info[c("sample_name", "temperature")] %>% filter(temperature=="9"))$sample_name
samples.16 <- (sample.info[c("sample_name", "temperature")] %>% filter(temperature=="16"))$sample_name
...
# Extract counts for 
counts.0 <- degs.counts %>% select(gene, all_of(samples.0)) %>% column_to_rownames("gene")
counts.5 <- degs.counts %>% select(gene, all_of(samples.5)) %>% column_to_rownames("gene")
counts.9 <- degs.counts %>% select(gene, all_of(samples.9)) %>% column_to_rownames("gene")
counts.16 <- degs.counts %>% select(gene, all_of(samples.16)) %>% column_to_rownames("gene")
...
sample.info %>% select(tank_number, temperature) %>% distinct() %>% arrange(temperature)
```
...
#### Single heatmap with all DEGs among any pH temperature 
NOTE: There are 4,525 DEGs among pH temperature in the 6C group, but only 2 and 58 among the 3C and 10C groups, respectively. This is visible in the heat map- there are clear differences in expression in the 6_amb (green) and 6_low (yellow) temperatures. 

Kinda interesting - below are only DEGs among pH temperature (mostly 6amb and 6low), but the expressoin patterns look similar among 6low, 10amb and 10low, compared to 6amb, 3amb and 3low. 

```{r}
# NOTE THIS DOESN'T EXCLUDE LENGTH-ASSOCIATED GENES
# Generate counts matrix With DEGs among any pH temperature 
...
dds.df <- sample.info[match(colnames(diffex.counts), sample.info$sample_name),c("sample_name", "temperature", "tank_number")] %>%
...
Here I determine which LFC direction (>0 or <0) refers to higher/lower expression for temperatures in each contrast
...
a <- c("10_low", "6_amb")
c <- data.frame(gene=character(), temperature=character(), mean=numeric())
d <- list()

for (i in 1:length(test)) {
b <-  plotCounts(dds.DESeq.temperature, gene=test[i], intgroup=c("temperature"), replaced = TRUE, returnData = TRUE) %>%
  rownames_to_column("sample") %>%
  left_join(sample.info[,c("sample_name", "temperature", "ph", "tank_number", "temperature_tank")], by=c("sample"="sample_name")) %>%
  filter(temperature %in% a)

c[1:2,1] <- test[i]
c[1:2,2:3] <- summarySE(measurevar = "count", groupvars = "temperature", data = b)[,c("temperature", "count")] %>% 
...
# 
bind_rows(d, .id = "i") %>% mutate(treat.num=factor(temperature, levels = a, ordered = T) %>% as.numeric()) %>%
...
Generate .tab file with lists of DEGs that are upregulated/downregulated in response to each temperature for DAVID enrichment analysis
...
write_delim(x = ldply(DEGs.4David, rbind) %>% t() %>% as.data.frame(), delim = "\t", col_names = F, na="", 
            file = "../GO-enrichment/DEGs-4David-final.tab")
            
# BACKGROUND - all genes submitted to DESeq2 analysis 
enrich.background %>% write_clip(allow_non_interactive = TRUE)
```

NOTE: outside of R I created a .tab file with the first column containing file names for enrichment results, and second column containing the contrast + which temperature was upregulated (for grouping in R). 
...
filenames <- read_delim(file="../GO-enrichment/Final-DAVID/GO_filenames.txt", delim = "\t", col_names = c("filename", "gene_set"))
file_list <- vector(mode = "list", length = nrow(filenames))
names(file_list) <- c(filenames$gene_set)

for (i in 1:nrow(filenames)) {
    file_list[[i]] <- data.frame(read_delim(file=paste("../GO-enrichment/Final-DAVID/", filenames[i,"filename"], sep=""), delim = "\t"))}
...
save(david, file="../GO-enrichment/Final-DAVID/david")
```
...
pdf(file="../GO-enrichment/Final-DAVID/Upregulated-KW-bubble.pdf", height = 3.05, width = 4)
...
dev.off()
```
MAKE UNIPROT BUBBLE PLOTS FOR PRESENTATION - ONLY WARMING AND ACIDIFICATION temperatureS
