...
```{r}
dxr1 %>% as.data.frame() %>%
  filter(groupID == "LOC132447235")

plotDEXSeq( dxr1, "LOC132447235", expression=T, displayTranscripts = F, norCounts=FALSE, 
            legend=TRUE, cex.axis=1.2, cex=1.3, lwd=2, splicing = F)

plotDEXSeq( dxr1, "LOC132447235", expression=FALSE, displayTranscripts = TRUE, norCounts=TRUE, legend=TRUE, cex.axis=1.2, cex=1.3, lwd=2 )
```
...
Effect of warming (16C vs. 9C)

```{r}
plotMA(dxr.treats$dxr.16vs9, cex=0.5, alpha=0.05, ylim=c(-5,5), xlim=(c(1, 1e6)),
       main="Differenitally used exons, 16C vs. control (9C)")
```
Effect of cool temperature (5C vs. 9C)

```{r}
plotMA(dxr.treats$dxr.5vs9, cex=0.5, alpha=0.05, ylim=c(-5,5), xlim=(c(1, 1e6)),
       main="Differenitally used exons, 5C vs. control (9C)")
```

Effect of cold temperature (0C vs. 9C)

```{r}
plotMA(dxr.treats$dxr.0vs9, cex=0.5, alpha=0.05, ylim=c(-5,5), xlim=(c(1, 1e6)),
       main="Differenitally used exons, 0C vs. control (9C)")
```

#### Focusing on the effect of Warming
...
```{r}
deu.0.annot %>% filter(groupID=="LOC132447235")

deu.16.annot <- dxr.treats$dxr.16vs9 %>% as.data.frame() %>% filter(padj<0.05) %>% # & abs(log2fold_d_16_a_9)>.5
  left_join(pcod.blast.GO, by=c("groupID"="gene")) %>%
  arrange(padj)

deu.5.annot <- dxr.treats$dxr.5vs9 %>% as.data.frame() %>% filter(padj<0.05) %>% # & abs(log2fold_d_16_a_9)>.5
  left_join(pcod.blast.GO, by=c("groupID"="gene")) %>%
  arrange(padj)

deu.0.annot <- dxr.treats$dxr.0vs9 %>% as.data.frame() %>% filter(padj<0.05) %>% # & abs(log2fold_d_16_a_9)>.5
  left_join(pcod.blast.GO, by=c("groupID"="gene")) %>%
  arrange(padj)

require(clipr)

DEU.4David.uniprot <- list(
  ...
  "Warm-LFC0.5" = 
  (deu.16.annot %>% filter(abs(log2fold_d_16_a_9)>.5) %>%
    remove_rownames() %>%
    dplyr::select(groupID, spid) %>% distinct() %>%
    filter(!is.na(spid)))$spid,
  
  "Cool-LFC0.5" = 
  (deu.5.annot %>% filter(abs(log2fold_c_5_a_9)>.5) %>%
    remove_rownames() %>%
    dplyr::select(groupID, spid) %>% distinct() %>%
    filter(!is.na(spid)))$spid,
  
  "Cold-LFC0.5" = 
  (deu.0.annot %>% filter(abs(log2fold_b_0_a_9)>.5) %>%
    remove_rownames() %>%
    dplyr::select(groupID, spid) %>% distinct() %>%
    filter(!is.na(spid)))$spid,

  "Warm-NoDEGs" = 
  (deu.16.annot %>% filter(groupID %!in% rownames(diffex.16.filt)) %>%
    remove_rownames() %>%
    dplyr::select(groupID, spid) %>% distinct() %>%
    filter(!is.na(spid)))$spid,
  
  "Cool-NoDEGs" = 
  (deu.5.annot %>% filter(groupID %!in% rownames(diffex.5.filt)) %>%
    remove_rownames() %>%
    dplyr::select(groupID, spid) %>% distinct() %>%
    filter(!is.na(spid)))$spid,
  
  "Cold-NoDEGs" = 
  (deu.0.annot %>% filter(groupID %!in% rownames(diffex.0.filt)) %>%
    remove_rownames() %>% 
    dplyr::select(groupID, spid) %>% distinct() %>%
    filter(!is.na(spid)))$spid
)
...
write_delim(x = plyr::ldply(DEU.4David.uniprot, rbind) %>% t() %>% as.data.frame(), delim = "\t", col_names = F, na="", 
            file = "../dexseq/DEU-4David.uniprot.txt")

# All genes for background (copy to clipboard), here I use the 16C dataframe but all contrasts have same background set so any would work 
...
```{r}
filenames <- read_delim(file="../dexseq/GO_filenames.txt", delim = "\t", col_names = c("filename", "gene_set"))
file_list <- vector(mode = "list", length = nrow(filenames))
names(file_list) <- c(filenames$gene_set)

for (i in 1:nrow(filenames)) {
    file_list[[i]] <- data.frame(read_delim(file=paste("../dexseq/", filenames[i,"filename"], sep=""), delim = "\t"))}

david.deu <- bind_rows(file_list, .id = "gene_set") %>% clean_names() %>%  # Bind dataframe for each gene_set together
...
save(david.deu, file="../dexseq/david")
```
...
#  filter(category=="GOTERM_BP_DIRECT") %>% 
  ...
  left_join(david.deu %>% 
              dplyr::select(stress, filtering, gene_set, category, term, process, count, percent, p_value, fold_enrichment, fdr, genes), 
            by = c("stress", "filtering", "genes", "p_value", "count")) %>%  #re-add data
#  ungroup() %>% arrange(stress, filtering, p_value) %>% group_by(stress, response) %>% dplyr::slice(1:15) %>%
...
ggplot(aes(y = process, x=stress:filtering, col=stress, alpha=filtering)) + 
...
DEG.kathleen <- read_delim("../Gmac_DEGs_sig_L.9.16_norm.tab")[-1] %>% arrange(padj)
...
Which genes containg DETs (Steven) AND DEUs (Laura) excluding DEGs?

```{r, message=F}
Reduce(intersect, list(`DEUs (Laura)`, `DETs (Steven)`, `DETs (Sam)`)) %>%
  as.data.frame() %>% `colnames<-`("gene") %>%
  left_join(pcod.blast.GO) %>% dplyr::select(gene, ncbi_id, protein, spid, evalue, species, gene_ontology_biological_process) %>% write_clip()

Reduce(intersect, list(`DEUs (Laura)`, `DETs (Steven)`)) %>%
  as.data.frame() %>% `colnames<-`("gene") %>%
  left_join(pcod.blast.GO)  %>% dplyr::select(gene, ncbi_id, protein, spid, evalue, species, gene_ontology_biological_process) %>% write_clip()
```
